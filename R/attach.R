...
alzverse_attach_message <- function(to_load) {
  if (length(to_load) == 0) {
    return(NULL)
  }

  header <- cli::rule(
    left = cli::style_bold("Attaching core alzverse packages"),
    right = paste0("alzverse ", package_version_h("alzverse"))
  )
